...
@Slf4j
public class HoubiSourceConnector implements SourceConnector {
    ...
    public void push(List<AggregationPrice> data) {
        for(AggregationPrice item: data){
            Pair<String, AggregationPrice> pair = new ImmutablePair<>(AggregatedSource.Huobi.name(), item);
            var event = kafkaEventTransformer.transform(pair);
            int partition = partitionStrategy.partition(AggregatedSource.Huobi);
    ...
    void sendWithRetry(ProducerRecord<String, PriceEventMessage> producerRecord){
        ...
        future.whenComplete((
                result, ex) -> {
            if (ex == null) {
                log.debug("Source data {} success",AggregatedSource.Huobi.name());
            } else {
